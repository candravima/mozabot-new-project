  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        if (cmd.name) embed.addField("**<:ArrowRR:867324401015980063> Command name**", `\`${cmd.name}\``);
        if (cmd.name) embed.setTitle(`<:ArrowRR:867324401015980063> Detailed Information about: \`${cmd.name}\``);
        if (cmd.description) embed.addField("**<:ArrowRR:867324401015980063> Description**", `\`\`\`${cmd.description}\`\`\``);
        if (cmd.aliases) try {
          embed.addField("**<:ArrowRR:867324401015980063> Aliases**", `\`${cmd.aliases.map((a) => `${a}`).join("`, `")}\``);
        } catch {}
        if (cmd.cooldown) embed.addField("**<:ArrowRR:867324401015980063> Cooldown**", `\`\`\`${cmd.cooldown} Seconds\`\`\``);
        else embed.addField("**<:ArrowRR:867324401015980063> Cooldown**", `\`\`\`3 Seconds\`\`\``);
        if (cmd.usage) {
          embed.addField("**<:ArrowRR:867324401015980063> Usage**", `\`\`\`${config.prefix}${cmd.usage}\`\`\``);
          embed.setFooter("Syntax: <> = required, [] = optional", es.footericon);
        }
        if (cmd.useage) {
          embed.addField("**<:ArrowRR:867324401015980063> Useage**", `\`\`\`${config.prefix}${cmd.useage}\`\`\``);
          embed.setFooter("Syntax: <> = required, [] = optional", es.footericon);
        }
        return message.channel.send(embed);
      } else {
        ...
        let button_invite = new MessageButton().setStyle('url').setLabel('Invite MozA Bot').setURL("https://discord.com/api/oauth2/authorize?client_id=813301564986097706&permissions=8&scope=bot");
        ...
        let button_cat_advertisement = new MessageButton().setStyle('blurple').setID('button_cat_advertisement').setLabel('​Advertisement').setEmoji("867644625648222220")
        ...
        .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        .setFooter("Page Home\n"+ client.user.username + " | Made by: AkuCandra (Discord)", client.user.displayAvatarURL())
        .setTitle(`Information about the <:mozabot:867644625648222220> MozA Bot: __**${client.user.username}**__`)
        .addField(":muscle: **__My Features__**",
`>>> **58+ Systems**, like: <:twitter:865965667241033788> **Twitter-** & <:Youtube:865965667370795028> **Youtube-Auto-Poster** 
**Application-**, Ticket-, **Welcome-Images-** and Reaction Role-, ... Systems
:notes: An advanced <:Spotify:865965667559669820> **Music System** with **Audio Filtering**
...
.addField("<:stats:866155255603265556> **__STATS:__**",
`>>> <:__:866155255644291083> **${client.commands.map(a=>a).length} Commands**
:file_folder: on **${client.guilds.cache.size} Guilds**
<:time:866155257775259689> **${duration(client.uptime).map(i=> `\`${i}\``).join(", ")} Uptime**
📶 **\`${Math.floor(client.ws.ping)}ms\` Ping**`)        
        ...
        let d_button_invite = new MessageButton().setStyle('url').setLabel('Invite MozA Bot').setURL("https://discord.com/api/oauth2/authorize?client_id=813301564986097706&permissions=8&scope=bot");
        ...
        let d_button_cat_advertisement = new MessageButton().setStyle('blurple').setID('button_cat_advertisement').setLabel('​Advertisement').setEmoji("867751518639095808").setDisabled(true);
        //array of all buttons
        

        let d_buttonRow1 = new MessageActionRow()
          .addComponent(d_button_back).addComponent(d_button_home).addComponent(d_button_forward)
          .addComponent(d_button_invite)
        ...
        function allotherembeds(){
          ...
          var embed0 = new MessageEmbed()
            ...
            .addField(settings.ECONOMY ? "💸 **Economy** | <a:yes:865963544380964864> ENABLED" : "💸 **Economy** | <a:no:865963806483808256> DISABLED",`> ${client.commands.filter((cmd) => cmd.category === "💸 Economy").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
            .addField(settings.SCHOOL ? "🏫 **School** | <a:yes:865963544380964864> ENABLED" : "🏫 **School** | <a:no:865963806483808256> DISABLED", `> ${client.commands.filter((cmd) => cmd.category === "🏫 School Commands").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
            .setFooter(`Page 1 / 9  | MozA\nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
          embeds.push(embed0)
          var embed1 = new MessageEmbed()
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
            .setTitle(`🎶 Music Related Commands :notes:`)
            .setDescription(`🎶 **Music**${settings.MUSIC ? " | <a:version:867324402151718932> STAGE OF WORK" : " | <a:no:865963806483808256> DISABLED"}\n> ${client.commands.filter((cmd) => cmd.category === "🎶 Music").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
            .addField(settings.MUSIC ? "👀 **Filter** | <a:version:867324402151718932> STAGE OF WORK" : "👀 **Filter** | <a:no:865963806483808256> DISABLED", `>>> ${client.commands.filter((cmd) => cmd.category === "👀 Filter").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
            .addField(settings.MUSIC ? "⚜️ **Custom Queue(s)** | <a:yes:865963544380964864> ENABLED" : "⚜️ **Custom Queue(s)** | <a:no:865963806483808256> DISABLED", `${client.commands.filter((cmd) => cmd.category === "⚜️ Custom Queue(s)").map((cmd) => `\`${cmd.name}\``).join(", ")}`.substr(0, 1024))
            .setFooter(`Page 2 / 9 \nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
          embeds.push(embed1)
          ...
          var embed4 = new MessageEmbed()
            ...
            .setDescription(`🎤 **Voice**${settings.VOICE ? " | <a:yes:865963544380964864> ENABLED" : " | <a:no:865963806483808256> DISABLED"}\n> ${client.commands.filter((cmd) => cmd.category === "🎤 Voice").map((cmd) => `**Command:**\n>>> \`${cmd.name}\`\n\n**Usage:**\n ${cmd.usage}`)}`)
            .addField("📈 **Ranking**", `>>> ${client.commands.filter((cmd) => cmd.category === "📈 Ranking").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
            .addField(settings.SOUNDBOARD ? "🔊 **Soundboard** | <a:yes:865963544380964864> ENABLED" : "🔊 **Soundboard** | <a:no:865963806483808256> DISABLED", `${client.commands.filter((cmd) => cmd.category === "🔊 Soundboard").map((cmd) => `\`${cmd.name}\``).join(", ")}`.substr(0, 1024))
            .setFooter(`Page 5 / 9  \nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
          embeds.push(embed4)
          var embed5 = new MessageEmbed()
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
            .setTitle(`🎮 Mini Games & Fun Commands 🕹️`)
            .setDescription(`🕹️ **Fun**${settings.FUN ? " | <a:yes:865963544380964864> ENABLED" : " | <a:no:865963806483808256> DISABLED"}\n> ${client.commands.filter((cmd) => cmd.category === "🕹️ Fun").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
            .addField(settings.MINIGAMES ? "🎮 **Mini Games** | <a:yes:865963544380964864> ENABLED" : "🎮 **Mini Games**| <a:no:865963806483808256>DISABLED", `> ${client.commands.filter((cmd) => cmd.category === "🎮 MiniGames").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
            .setFooter(`Page 6 / 9  \nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
          embeds.push(embed5)
          var embed6 = new MessageEmbed()
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
            .setTitle(settings.NSFW ? "🔞 NSFW Commands 🔞 | <a:yes:865963544380964864> ENABLED" : "🔞 NSFW Commands 🔞 | <a:no:865963806483808256> DISABLED")
        ...
        var embed8 = new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setTitle("<a:Discord:865978943055462440> Bot Creator")
        
          .addField(`<a:Discord:865978943055462440> **__Bot Creator Information__**`,`>>> 💯 This Bot has been made by AkuCandra#8450
          \n[Discord](https://discord.gg/pZAbVuH) <a:Discord:865978943055462440> \n[Donate](https://saweria.co/Candravima) <:economy:866155254613147658> `) 
          .addField(`<:fun:866155255321853982>`,`**THANK YOU FOR USING THIS BOT**`)   
          .setFooter(`Page 9 / 9  \nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
        embeds.push(embed8)
